...
from aif360.sklearn.metrics.metrics import average_odds_difference,statistical_parity_difference,equal_opportunity_difference 
...
def compute_y(X,metric,prot_attr,model):
    metrics = {'size':[],'values':[]}
    X['bins_0'] = bin_codes_0 + 1
    X['bins_1'] = bin_codes_1 + 1
    X = X.set_index(['bins_0','bins_1'])
    
    for index, x in X.groupby(level=[0, 1]):
        pred = model.predict(x.loc[:, x.columns != 'labels'])
        x = x.set_index(prot_attr)
        metrics['size'].append(len(x))
        metrics['values'].append(metric(x.labels, pred, prot_attr=prot_attr))
    return metrics


def faleplot_2D_continuous(X, model, features,metric,prot_attr,grid_size=9):
    ...
    # reset index to avoid index missmatches when replacing the values with the codes 
    ...
    bins_0 =  np.array(sorted(set(np.percentile(X[features[0]].to_numpy(), percentiles))))
    ...
    bins_1 =  np.array(sorted(set(np.percentile(X[features[1]].to_numpy(), percentiles))))
    ...
    y_11 = np.asarray(compute_y(X11,metric,prot_attr,model)['values'])
    y_12 = np.asarray(compute_y(X12,metric,prot_attr,model)['values'])
    y_21 = np.asarray(compute_y(X21,metric,prot_attr,model)['values'])
    y_22 = np.asarray(compute_y(X22,metric,prot_attr,model)['values'])

    X['bins_0'] = [i+1 for i in bin_codes_0]
    X['bins_1'] = [i+1 for i in bin_codes_1]

    X = X.set_index(['bins_0','bins_1'])
    X = X.sort_values(by = ['bins_0','bins_1'])

    delta_df = pd.DataFrame(
        {
            "mean": (y_22 - y_21) - (y_12 - y_11),
            "size": [len(X.loc[index]) for index in X.index.unique()]
        },
...
def plot_2D_continuous_eff(eff_grid, metric, prot_attr, contour=False, fig=None, ax=None):
    ...
    if fig is None and ax is None:
        fig, ax = plt.subplots()
    imc = ax.contourf(X, Y, eff_grid, cmap="bwr", levels=30, alpha=1)
    cbar = plt.colorbar(imc)

    ax.set_xlabel(eff_grid.columns.name)
    ax.set_ylabel(eff_grid.index.name)
    fig.suptitle(f"2D FALE Plot for protected attribute {prot_attr} using fairness metric {metric.__name__} ")

    cbar.ax.set_ylabel("Effect on fairness", rotation=270, labelpad=25)
    return fig, ax